...
def greedy_decode(model: nn.Module, source: torch.Tensor, source_mask: torch.Tensor, 
    ...
    """
    Perform greedy decoding on the source sequence.

    Args:
        model (nn.Module): The transformer model.
        source (torch.Tensor): The source sequence.
        source_mask (torch.Tensor): The mask for the source sequence.
        tokenizer_src (Tokenizer): The source tokenizer.
        tokenizer_tgt (Tokenizer): The target tokenizer.
...
def run_validation(model: nn.Module, validation_ds: DataLoader, tokenizer_src: Tokenizer, 
                   tokenizer_tgt: Tokenizer, max_len: int, device: torch.device, 
                   print_msg: Callable[[str], None], global_step: int, 
                   writer: SummaryWriter, num_examples: int = 2):
    """
    ...
    Args:
        ...
        max_len (int): The maximum length of the decoded sequence.
        device (torch.device): The device to perform computations on.
        print_msg (Callable[[str], None]): A function to print messages.
        global_step (int): The current global step.
        writer (SummaryWriter): The TensorBoard SummaryWriter.
        num_examples (int, optional): Number of examples to print. Defaults to 2.
    """
    model.eval()
    count = 0
...
def get_all_sentences(ds, lang: str):
    """
    Generator function to get all sentences for a given language from the dataset.
...
def get_or_build_tokenizer(config: dict, ds, lang: str) -> Tokenizer:
    """
    Get an existing tokenizer or build a new one if it doesn't exist.
...
def get_ds(config: dict) -> Tuple[DataLoader, DataLoader, Tokenizer, Tokenizer]:
    """
    Prepare the dataset and tokenizers.
    ...
    Returns:
        Tuple[DataLoader, DataLoader, Tokenizer, Tokenizer]: 
            Train dataloader, validation dataloader, source tokenizer, target tokenizer.
    """
...
def train_model(config: dict):
    ...
    device = ("cuda" if torch.cuda.is_available() else 
              "mps" if torch.has_mps or torch.backends.mps.is_available() else 
              "cpu")
    ...
    if device == 'cuda':
        ...
        print("NOTE: For better performance, consider using a GPU.")
